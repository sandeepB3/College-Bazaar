...
//To render approved items in main page
...
//Finds the user adding items and then renders the found items in addItems.ejs
...
router.post('/addItem', isLoggedIn,isBlacklisted,upload.array('image'), async (req, res) => {
    ...
    await newItem.save()
    req.flash('success', 'Item succesfully uploaded awaiting for approval!')
...
//Search Route to search items
router.post('/tags', wrapAsync(async (req, res) => {
    const searchName = _.capitalize(req.body.name)
